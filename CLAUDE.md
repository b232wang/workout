...
6. **记录类确认后可自动 push,但前 5 条是前提**:记录健身 / 食物时,只要数据我没有疑问、或不确定处已摊出原话经用户回复确认,且 totals 已真算无误,即可直接 `git add <对应数据文件> && commit && push` 上线,**无需再单独问是否推送**(用户 2026-06-29 明确授权,认可"不确定就问、确认后执行"的流程,只是免去每次问 push)。**但凡还有未确认的疑问、或引不出用户原话(违反铁律 1/2),立即停下问,绝不带着编造风险 push。** 只 commit 本次改动的相关文件,不裹入无关改动。**前端代码改动(`index.html` 等)与数据改动一样,确认无误后直接 `commit && push` 上线、无需询问**(用户 2026-07-12 明确要求:「这个 project 每次前端或者添加食材都要自动上线」)。仅「主动改本 CLAUDE.md 规约」这类元变更仍先确认。
...
## health-sync(Apple Health 自动导入 · 个人管线)

- `health-sync/backend/` — 部署在 NAS 的接收端,收 Vitaduct app(独立产品,见 memory `vitaduct-project`)推来的 Apple Health 数据。
- **Phase 3(Mac 同步,未开工)**:Mac 定时从 NAS 拉健身活动 → 自动写成 `cardio`/`walk` 条目上网页。设计 + 开工前待确认项见 `health-sync/PHASE3.md`;用户说「开始 phase3」时先读它。
